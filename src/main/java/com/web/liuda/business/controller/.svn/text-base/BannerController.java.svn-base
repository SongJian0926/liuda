...
public class BannerController extends BaseController {
	...
	/**
	 * @Title: findBannerNEStatusPage
	 * @Description: 分页查询Banner	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findBannerNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Banner>> findBannerNEStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		@RequestParam(defaultValue = "3") Integer status,
		@RequestParam(defaultValue = "[{property:'sort',direction:'ASC'}]") String sortData,
		...
		return bannerService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findBannerEQStatusPage
	 * @Description: (预留)分页查询Banner	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findBannerEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Banner>> findBannerEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return bannerService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	
	@ResponseBody
	@RequestMapping(value="findBannerSort",method=RequestMethod.POST)
	public XaResult<List<Banner>> findBannerSort(
		...
		return bannerService.findBannerSort(status, id);
	}
	
	@ResponseBody
	@RequestMapping(value="checkRole",method=RequestMethod.POST)
	public String checkRoleNameExist(@RequestParam(required=true) Integer sort,@RequestParam(required=false) Long id){
		Banner banner = bannerService.findSortBySort(sort);
		if(banner==null || id==banner.getId()){
			return "{\"ok\":\"可以使用\"}";
		}else{
			return  "{\"error\":\"已存在\"}";
		}
	...
	public XaResult<Banner> findBannerById(
		@RequestParam Long modelId
	) throws BusinessException{
		return bannerService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<Banner> saveBanner(
		Banner model
	) throws BusinessException{
		return bannerService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateBannerByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateBannerByIds",method=RequestMethod.POST)
	public XaResult<Banner> operateBannerByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return bannerService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/banner";
