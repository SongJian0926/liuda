...
/**
 * @Title: ClubMemberController.java
 * @Package com.web.webstart.business.controller
 * @Description: 俱乐部成员控制器
 * @author eason.zt
 * @date 2015年3月23日 下午1:00:00
 * @version V1.0
 */
@Controller
@RequestMapping("/cms/clubMember")
public class ClubMemberController extends BaseController {
	...
	/**
	 * @Title: findClubMemberNEStatusPage
	 * @Description: 分页查询ClubMember	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findClubMemberNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<ClubMember>> findClubMemberNEStatusPage(
		...
		return clubMemberService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findClubMemberEQStatusPage
	 * @Description: (预留)分页查询ClubMember	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findClubMemberEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<ClubMember>> findClubMemberEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return clubMemberService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	...
	public XaResult<ClubMember> findClubMemberById(
		@RequestParam Long modelId
	) throws BusinessException{
		return clubMemberService.findOne(modelId);
	}

	/**
	 * @Title: findClubMemberByClubIdPage
	 * @Description: 分页查询ClubMember	 * @param nextPage
	 * @param pageSize
	 * @param status		//默认查询非删除3的数据,具体类型参照XaConstant.Status
	 * @param sortDate
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findClubMemberByClubIdPage",method=RequestMethod.POST)
	public XaResult<Page<ClubMemberVo>> findClubMemberByClubIdPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		@RequestParam(defaultValue = "3") Integer status,
		@RequestParam(defaultValue = "[{property:'createTime',direction:'DESC'}]") String sortData,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return clubMemberService.findClubMemberByClubIdPage(status, filterParams, pageable);
	}
	...
	public XaResult<ClubMember> saveClubMember(
		ClubMember model
	) throws BusinessException{
		return clubMemberService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateClubMemberByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateClubMemberByIds",method=RequestMethod.POST)
	public XaResult<ClubMember> operateClubMemberByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return clubMemberService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/clubMember";
