 ...
 */
@Controller
@RequestMapping("/cms/macth")
public class MacthController extends BaseController {

	@Autowired
	private MacthService macthService;
	
	/**
	 * @Title: findMacthNEStatusPage
	 * @Description: 分页查询Macth	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findMacthNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Macth>> findMacthNEStatusPage(
		...
		return macthService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findMacthEQStatusPage
	 * @Description: (预留)分页查询Macth	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findMacthEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Macth>> findMacthEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return macthService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findMacthById",method=RequestMethod.POST)
	public XaResult<MacthVo> findMacthById(
		@RequestParam Long modelId
	) throws BusinessException{
		XaResult<MacthVo> vo = macthService.findMacthById(modelId);
		return vo;
	}
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="saveMacth",method=RequestMethod.POST)
	public XaResult<Macth> saveMacth(
			@RequestParam Long id,
			@RequestParam Long businessUserId,
			@RequestParam String areaName,
			@RequestParam String guessJson,
			Macth model
	) throws BusinessException{
//		model.setId(id);
//		model.setBusinessUserId(businessUserId);
		model.setAreaCode(areaName);
		GuessVo guess = null;
		if(!guessJson.equals(""))
		{
			try {
				guess = JSON.parseObject(URLDecoder.decode(guessJson, "UTF-8"),GuessVo.class);
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
		}
		return macthService.saveOrUpdate(model,guess);
	}
	
	/**
	 * @Title: multiOperateMacthByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateMacthByIds",method=RequestMethod.POST)
	public XaResult<Macth> operateMacthByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return macthService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/macth";
	...
	/**
	 * @Title: upAndDownMatch
	 * @Description:后台赛事的上架或者下架或者审批
	 * @param matchIds 赛事ID列表,以逗号分隔
	 * @param status 1审批  2上架 －1下架
	 * author:flora  2016-05-20
	 * @return    
	 */
	@ResponseBody
	@RequestMapping(value="upAndDownMatch",method=RequestMethod.POST)
	public XaResult<Macth> upAndDownMatch(
		HttpServletRequest request,
		@RequestParam String matchIds,
		@RequestParam Integer status
	) throws BusinessException{
		return macthService.upAndDownMatch(matchIds, status);
	}
}
