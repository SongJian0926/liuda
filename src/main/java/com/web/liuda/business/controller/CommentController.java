...
public class CommentController extends BaseController {
	...
	/**
	 * @Title: findCommentNEStatusPage
	 * @Description: 分页查询Comment	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findCommentNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<CommentVo>> findCommentNEStatusPage(
		...
		return commentService.findPageNEStatusByFilter(filterParams, pageable);
	}
	
	/**
	 * author:changlu
	 * time:2016-01-12 17:00:00
	 * @Title: findCommentList
	 * @Description: pc端分页查询Comment	 * @param nextPage
	 * @param pageSize
	 * @param status		//默认查询非删除3的数据,具体类型参照XaConstant.Status
	 * @param sortDate
	 * @param jsonFilter
	...
	public XaResult<Page<CommentVo>> findCommentList(
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return commentService.findCommentByFilter(businessId,businessType,filterParams, pageable);
	}
	
	/**
	 * @Title: findCommentEQStatusPage
	 * @Description: (预留)分页查询Comment	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findCommentEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Comment>> findCommentEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return commentService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findCommentById",method=RequestMethod.POST)
	public XaResult<Comment> findCommentById(
		@RequestParam Long modelId
	) throws BusinessException{
		return commentService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<Comment> saveComment(
		Comment model
	) throws BusinessException{
		return commentService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateCommentByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateCommentByIds",method=RequestMethod.POST)
	public XaResult<Comment> operateCommentByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return commentService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/comment";
