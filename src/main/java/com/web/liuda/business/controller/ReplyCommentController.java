...
/**
 * @Title: ReplyCommentController.java
 ...
 */
@Controller
@RequestMapping("/cms/replyComment")
public class ReplyCommentController extends BaseController {
	...
	/**
	 * @Title: findReplyCommentNEStatusPage
	 * @Description: 分页查询ReplyComment	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findReplyCommentNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<ReplyComment>> findReplyCommentNEStatusPage(
		...
		return replyCommentService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findReplyCommentEQStatusPage
	 * @Description: (预留)分页查询ReplyComment	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findReplyCommentEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<ReplyComment>> findReplyCommentEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return replyCommentService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findReplyCommentById
	 * @Description: 根据ID查找单条实体
	 * @param modelId
	 * @return    
	 */
	@ResponseBody
	@RequestMapping(value="findReplyCommentById",method=RequestMethod.POST)
	public XaResult<ReplyComment> findReplyCommentById(
		@RequestParam Long modelId
	) throws BusinessException{
		return replyCommentService.findOne(modelId);
	}
	/**
	 * author:changlu
	 * time:2016-01-13 16:04:00
	 * @Title: saveModel
	 * @Description: 保存实体数据
	 * @param model
	 * @return    
	...
	public XaResult<ReplyComment> saveReplyComment(
		ReplyComment model,
		@RequestParam(required=false) String pics1
	) throws BusinessException{
		XaResult<ReplyComment> xr=new XaResult<ReplyComment>();
		if(XaUtil.isEmpty(model.getBusinessUserId())||XaUtil.isEmpty(model.getCommentId())||XaUtil.isEmpty(model.getUserId())){
			xr.error("还有不符合要求的字段！");
			return xr;
		}
		return replyCommentService.newSaveOrUpdate(model,pics1);
	}
	
	/**
	 * @Title: multiOperateReplyCommentByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateReplyCommentByIds",method=RequestMethod.POST)
	public XaResult<ReplyComment> operateReplyCommentByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return replyCommentService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/replyComment";
