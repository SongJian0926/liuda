...
public class VoteController extends BaseController {
	...
	/**
	 * @Title: findVoteNEStatusPage
	 * @Description: 分页查询Vote	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findVoteNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Vote>> findVoteNEStatusPage(
		...
		return voteService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findVoteEQStatusPage
	 * @Description: (预留)分页查询Vote	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findVoteEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Vote>> findVoteEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return voteService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findVoteByIds",method=RequestMethod.POST)
	public XaResult<VoteVo> findVoteByIds(
		@RequestParam Long modelId
	) throws BusinessException{
		return voteService.findOnes(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<Vote> saveVote(
		Vote model
	) throws BusinessException{
		return voteService.saveOrUpdate(model);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="saveVoteing",method=RequestMethod.POST)
	public XaResult<VoteVo> saveVoteing(
		Vote model,
		String standard
	) throws BusinessException{
		...
		try{
			voteOptions = JSON.parseArray(standard, VoteOption.class);
		}catch(Exception e){
			xr.error("json格式转化异常");
			return xr;
		}
		model.setStatus(XaConstant.Status.publish);
		return voteService.saveVoteing(model, voteOptions);
	}
	
	/**
	 * @Title: multiOperateVoteByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateVoteByIds",method=RequestMethod.POST)
	public XaResult<Vote> operateVoteByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return voteService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/vote";
