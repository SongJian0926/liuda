...
public class CityController extends BaseController {
	...
	/**
	 * @Title: findCityByProvinceCode
	 * @Description: 分页查询City	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findCityByProvinceCode",method=RequestMethod.POST)
	public XaResult<List<City>> findCityByProvinceCode(
		...
		return cityService.findCityByProvinceCode(status, provinceCode);
	}

	/**
	 * @Title: findCityByCityCode
	 * @Description: 分页查询City	 * @param nextPage
	 * @param pageSize
	 * @param status		//默认查询非删除3的数据,具体类型参照XaConstant.Status
	 * @param sortDate
	...
	public XaResult<City> findCityByCityCode(
		@RequestParam(defaultValue = "3") Integer status,
		@RequestParam(defaultValue = "") String cityCode
		//过滤字段,字段名:jsonFilter,选填,默认:{},示例:{'search_EQ_field1':'value1','search_EQ_field2':'value2'},字段名称拼接规则search_为固定查询标识,EQ为等于,filed为字段名
		//EQ等于, IN包含, ISNULL空, LIKE, GT大于, LT小于, GTE大于等于, LTE小于等于, NE不等于,LIKEIGNORE非like 
	) throws BusinessException{
		return cityService.findCityByCityCode(status, cityCode);
	}
	
	/**
	 * @Title: findCityNEStatusPage
	 * @Description: 分页查询City	 * @param nextPage
	 * @param pageSize
	 * @param status		//默认查询非删除3的数据,具体类型参照XaConstant.Status
	 * @param sortDate
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findCityNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<City>> findCityNEStatusPage(
		...
		return cityService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findCityEQStatusPage
	 * @Description: (预留)分页查询City	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findCityEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<City>> findCityEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return cityService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	...
	public XaResult<City> findCityById(
		@RequestParam Long modelId
	) throws BusinessException{
		return cityService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<City> saveCity(
		City model
	) throws BusinessException{
		return cityService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateCityByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateCityByIds",method=RequestMethod.POST)
	public XaResult<City> operateCityByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return cityService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/city";
