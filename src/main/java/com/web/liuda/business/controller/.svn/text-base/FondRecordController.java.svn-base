...
/**
 * @Title: FondRecordController.java
 ...
 */
@Controller
@RequestMapping("/cms/fondRecord")
public class FondRecordController extends BaseController {

	@Autowired
	private FondRecordService fondRecordService;
	
	/**
	 * @Title: findFondRecordNEStatusPage
	 * @Description:用户 提现记录列表
	 * author:changlu
	 * time:2016-05-09 14:05:00
	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findFondRecordNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<FondRecordVo>> findFondRecordNEStatusPage(
		...
		return fondRecordService.findFondRecordList(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findFondRecordEQStatusPage
	 * @Description: (预留)分页查询FondRecord	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findFondRecordEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<FondRecord>> findFondRecordEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return fondRecordService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findFondRecordById
	 * @Description: 根据ID查找单条实体
	 * @param modelId
	 * @return    
	 */
	@ResponseBody
	@RequestMapping(value="findFondRecordById",method=RequestMethod.POST)
	public XaResult<FondRecord> findFondRecordById(
		@RequestParam Long modelId
	) throws BusinessException{
		return fondRecordService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="saveFondRecord",method=RequestMethod.POST)
	public XaResult<FondRecord> saveFondRecord(
		FondRecord model
	) throws BusinessException{
		return fondRecordService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateFondRecordByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateFondRecordByIds",method=RequestMethod.POST)
	public XaResult<FondRecord> operateFondRecordByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return fondRecordService.multiOperate(modelIds,status);
	}
	/**
	...
	public XaResult<FondRecord> sendMoney(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(value="presentState") Integer presentState
	) throws BusinessException{
		return fondRecordService.changePresentState(modelIds,presentState);
	}
	/**
	 * @Title: upload
	 * @Description: 图片上传
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/fondRecord";
	...
	public void fileDownload(
		...
		Map<String, Object> filterParams = WebUitl.getParametersStartingWith(
				jsonFilter, "search_");
		fondRecordService.exportdata(filterParams, response);
	}
}
