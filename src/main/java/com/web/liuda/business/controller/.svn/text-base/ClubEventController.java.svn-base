...
/**
 * @Title: ClubEventController.java
 * @Package com.web.webstart.business.controller
 * @Description: 俱乐部活动控制器
 * @author eason.zt
 * @date 2015年3月23日 下午1:00:00
 * @version V1.0
 */
@Controller
@RequestMapping("/cms/clubEvent")
public class ClubEventController extends BaseController {

	@Autowired
	private ClubEventService clubEventService;
	
	/**
	 * @Title: findClubEventNEStatusPage
	 * @Description: 分页查询ClubEvent	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findClubEventNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<ClubEvent>> findClubEventNEStatusPage(
		...
		return clubEventService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findClubEventEQStatusPage
	 * @Description: (预留)分页查询ClubEvent	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findClubEventEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<ClubEvent>> findClubEventEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return clubEventService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	...
	public XaResult<ClubEvent> findClubEventById(
		@RequestParam Long modelId
	) throws BusinessException{
		return clubEventService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<ClubEvent> saveClubEvent(
		ClubEvent model
	) throws BusinessException{
		return clubEventService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateClubEventByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateClubEventByIds",method=RequestMethod.POST)
	public XaResult<ClubEvent> operateClubEventByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return clubEventService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/clubEvent";
	...
	/**
	 * @Title: applyClubEvent
	 * @Description:后台俱乐部活动的审批
	 * @param matchIds 俱乐部活动ID列表,以逗号分隔
	 * author:flora  2016-05-20
	 * @return    
	 */
	@ResponseBody
	@RequestMapping(value="applyClubEvent",method=RequestMethod.POST)
	public XaResult<ClubEvent> applyClubEvent(
		HttpServletRequest request,
		@RequestParam String eventIds,
		@RequestParam Integer status
	) throws BusinessException{
		return clubEventService.applyClubEvent(eventIds,status);
	}
}
