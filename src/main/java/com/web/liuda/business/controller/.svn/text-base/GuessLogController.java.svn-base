...
/**
 * @Title: GuessLogController.java
 * @Package com.web.webstart.business.controller
 * @Description: 赛事记录表控制器
 * @author eason.zt
 * @date 2015年3月23日 下午1:00:00
 * @version V1.0
 */
@Controller
@RequestMapping("/cms/guessLog")
public class GuessLogController extends BaseController {

	@Autowired
	private GuessLogService guessLogService;
	
	/**
	 * @Title: findGuessLogNEStatusPage
	 * @Description: 分页查询GuessLog	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findGuessLogNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<GuessLog>> findGuessLogNEStatusPage(
		...
		return guessLogService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findGuessLogEQStatusPage
	 * @Description: (预留)分页查询GuessLog	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findGuessLogEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<GuessLog>> findGuessLogEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return guessLogService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findGuessLogById
	 * @Description: 根据ID查找单条实体
	 * @param modelId
	 * @return    
	 */
	@ResponseBody
	@RequestMapping(value="findGuessLogById",method=RequestMethod.POST)
	public XaResult<GuessLog> findGuessLogById(
		@RequestParam Long modelId
	) throws BusinessException{
		return guessLogService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<GuessLog> saveGuessLog(
		GuessLog model
	) throws BusinessException{
		return guessLogService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateGuessLogByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateGuessLogByIds",method=RequestMethod.POST)
	public XaResult<GuessLog> operateGuessLogByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return guessLogService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/guessLog";
