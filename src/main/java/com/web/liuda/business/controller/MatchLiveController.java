...
/**
 * @Title: MatchLiveController.java
 ...
 */
@Controller
@RequestMapping("/cms/matchLive")
public class MatchLiveController extends BaseController {

	@Autowired
	private MatchLiveService matchLiveService;
	
	/**
	 * @Title: findMatchLiveNEStatusPage
	 * @Description: 分页查询MatchLive	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findMatchLiveNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<MatchLive>> findMatchLiveNEStatusPage(
		...
		return matchLiveService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findMatchLiveEQStatusPage
	 * @Description: (预留)分页查询MatchLive	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findMatchLiveEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<MatchLive>> findMatchLiveEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return matchLiveService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findMatchLiveById
	 * @Description: 根据ID查找单条实体
	 * @param modelId
	 * @return    
	 */
	@ResponseBody
	@RequestMapping(value="findMatchLiveById",method=RequestMethod.POST)
	public XaResult<MatchLive> findMatchLiveById(
		@RequestParam Long modelId
	) throws BusinessException{
		return matchLiveService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="saveMatchLive",method=RequestMethod.POST)
	public XaResult<MatchLive> saveMatchLive(
		MatchLive model
	) throws BusinessException{
		return matchLiveService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateMatchLiveByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateMatchLiveByIds",method=RequestMethod.POST)
	public XaResult<MatchLive> operateMatchLiveByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return matchLiveService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/matchLive";
	...
	public XaResult<String> videoUpload(
		...
		String root=request.getSession().getServletContext().getRealPath("/");
		String picturePath = "/upload/matchLive";
		String ext =videoFile.getOriginalFilename().substring(videoFile.getOriginalFilename().lastIndexOf("."));
		String newName = new Date().getTime()+ext;
		File filedict = new File(root+picturePath);
		if(!filedict.exists()){
			filedict.mkdir();
		...
		File targetFile=new File(root+picturePath+File.separator+newName);
		try {
			if(StringUtils.equalsIgnoreCase(ext, ".mp4") || StringUtils.equalsIgnoreCase(ext, ".mov")){
				...
				xr.setCode(0);
				xr.setObject(XaConstant.Code.error+":上传文件类型不允许,请上传mp4/mov视频");
				return xr;
			}
		} catch (IllegalStateException e) {
			xr.setCode(0);
			xr.setObject(XaConstant.Code.error+":视频上传失败");
			return xr;
		} catch (IOException e) {
			xr.setCode(0);
			xr.setObject(XaConstant.Code.error+":视频上传失败");
