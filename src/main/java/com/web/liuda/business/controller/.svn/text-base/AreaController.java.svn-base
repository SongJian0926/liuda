...
public class AreaController extends BaseController {
	...
	/**
	 * @Title: findAreaByCityCode
	 * @Description: 分页查询Area	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findAreaByCityCode",method=RequestMethod.POST)
	public XaResult<List<Area>> findAreaByCityCode(
		...
		return areaService.findAreaByCityCode(status, cityCode);
	}

	/**
	 * @Title: findAreaByAreaCode
	 * @Description: 分页查询Area	 * @param nextPage
	 * @param pageSize
	 * @param status		//默认查询非删除3的数据,具体类型参照XaConstant.Status
	 * @param sortDate
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findAreaByAreaCode",method=RequestMethod.POST)
	public XaResult<Area> findAreaByAreaCode(
		@RequestParam(defaultValue = "3") Integer status,
		@RequestParam(defaultValue = "") String areaCode
		//过滤字段,字段名:jsonFilter,选填,默认:{},示例:{'search_EQ_field1':'value1','search_EQ_field2':'value2'},字段名称拼接规则search_为固定查询标识,EQ为等于,filed为字段名
		//EQ等于, IN包含, ISNULL空, LIKE, GT大于, LT小于, GTE大于等于, LTE小于等于, NE不等于,LIKEIGNORE非like 
	) throws BusinessException{
		return areaService.findAreaByAreaCode(status, areaCode);
	}
	
	/**
	 * @Title: findAreaNEStatusPage
	 * @Description: 分页查询Area	 * @param nextPage
	 * @param pageSize
	 * @param status		//默认查询非删除3的数据,具体类型参照XaConstant.Status
	 * @param sortDate
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findAreaNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Area>> findAreaNEStatusPage(
		...
		return areaService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findAreaEQStatusPage
	 * @Description: (预留)分页查询Area	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findAreaEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Area>> findAreaEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return areaService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	...
	public XaResult<Area> findAreaById(
		@RequestParam Long modelId
	) throws BusinessException{
		return areaService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<Area> saveArea(
		Area model
	) throws BusinessException{
		return areaService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateAreaByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateAreaByIds",method=RequestMethod.POST)
	public XaResult<Area> operateAreaByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return areaService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/area";
