...
public class NoticeController extends BaseController {
	...
	/**
	 * @Title: findNoticeNEStatusPage
	 * @Description: 分页查询Notice	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findNoticeNEStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Notice>> findNoticeNEStatusPage(
		...
		return noticeService.findListNEStatusByFilter(status, filterParams, pageable);
	}
	
	/**
	 * @Title: findNoticeEQStatusPage
	 * @Description: (预留)分页查询Notice	 * @param nextPage
	 * @param pageSize
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="findNoticeEQStatusPage",method=RequestMethod.POST)
	public XaResult<Page<Notice>> findNoticeEQStatusPage(
		@RequestParam(defaultValue = "0") Integer nextPage,
		@RequestParam(defaultValue = "10") Integer pageSize,
		...
		Pageable pageable = WebUitl.buildPageRequest(nextPage, pageSize, sortData);
		Map<String,Object> filterParams =  WebUitl.getParametersStartingWith(jsonFilter, "search_");
		return noticeService.findListEQStatusByFilter(status, filterParams, pageable);
	}
	...
	public XaResult<Notice> findNoticeById(
		@RequestParam Long modelId
	) throws BusinessException{
		return noticeService.findOne(modelId);
	}
	/**
	 * @Title: saveModel
	 * @Description: 保存实体数据
	...
	public XaResult<Notice> saveNotice(
		Notice model
	) throws BusinessException{
		XaResult<Notice> xr=new XaResult<Notice>();
		if(XaUtil.isEmpty(model.getContent())){
			xr.error("内容不能为空！");
			return xr;
		}
		
		if(XaUtil.isEmpty(model.getImgPath())){
			xr.error("图片不能为空！");
			return xr;
		}
		if(XaUtil.isEmpty(model.getType())){
			xr.error("公告类型不能为空！");
			return xr;
		}
		return noticeService.saveOrUpdate(model);
	}
	
	/**
	 * @Title: multiOperateNoticeByIds
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="operateNoticeByIds",method=RequestMethod.POST)
	public XaResult<Notice> operateNoticeByIds(
		HttpServletRequest request,
		@RequestParam String modelIds,
		@RequestParam(defaultValue = "3") Integer status
	) throws BusinessException{
		return noticeService.multiOperate(modelIds,status);
	}
	...
	public XaResult<String> photoUpload(
		...
		String picturePath = "/upload/notice";
